...
public class ArtifactAssociationHandler {
    ...
    private static final String INIT_ERROR_MSG = "Failed while attaching workflow artifact to Operation in SDC. Parameters where not initialized";
    private static final Logger LOGGER = LoggerFactory.getLogger(ArtifactAssociationHandler.class);
    ...
    @Autowired
    public ArtifactAssociationHandler(RestTemplateBuilder builder) {
    ...
    ResponseEntity<String> execute(String userId, ArtifactDeliveriesRequestDto deliveriesRequestDto,
        ...
        if(!parametersInitialized()){
            ...
            return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(INIT_ERROR_MSG);
        }
    ...
    private boolean parametersInitialized() {
        return !sdcBeEndpoint.equals("") && !sdcBeProtocol.equals("") && !sdcUser.equals("") && !sdcPassword.equals("");
    }
