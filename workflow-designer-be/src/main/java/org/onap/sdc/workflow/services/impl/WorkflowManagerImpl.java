...
public class WorkflowManagerImpl implements WorkflowManager {
    ...
    @Override
    public void updateStatus(Workflow workflow, WorkflowStatus status) {
        Item item = workflowMapper.workflowToItem(workflow);
        try {
            switch (status) {
                case ARCHIVED:
            ...
            throw new WorkflowStatusModificationException(workflow.getId(),status);
        }
    }
  ...
  private static RequestSpec getRequestSpec(RequestSpec requestSpec) {
    ...
    private static Predicate<Item> addItemStatusFilter(Predicate<Item> filter, String itemStatusFilter) {
        if (itemStatusFilter == null)
            return filter.and(item -> item.getStatus().equals(ItemStatus.ACTIVE));
        else return filter.and(item -> item.getStatus().equals(ItemStatus.valueOf(itemStatusFilter)));
    }
}
