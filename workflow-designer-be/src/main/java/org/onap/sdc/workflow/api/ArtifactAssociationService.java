...
public class ArtifactAssociationService {
    ...
    private static final String INIT_ERROR_MSG = "Failed while attaching workflow artifact to Operation in SDC. Parameter was not initialized: %s";
    ...
    private Optional<String> parametersInitializationState() {
        if (sdcBeEndpoint.equals("")) {
            return Optional.of("SDC_ENDPOINT");
        }
        if (sdcBeProtocol.equals("")) {
            return Optional.of("SDC_PROTOCOL");
        }
        if (sdcUser.equals("")) {
            return Optional.of("SDC_USER");
        }
        if (sdcPassword.equals("")) {
            return Optional.of("SDC_PASSWORD");
        }
